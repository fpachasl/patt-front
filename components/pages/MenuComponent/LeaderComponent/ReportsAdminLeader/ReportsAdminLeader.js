...
import { fetchReportKPIS, fetchReportSumary } from "@/lib/services/projects";
...
export default function ReportsAdminLeader(token) {
	return (
		<div className="w-full">
			<div className="flex pb-2 w-full">
				<Button
					onClick={() => {
						fetchReportSumary(token.token);
					}}
					className="flex flex-wrap px-2 bg-green-400 w-full h-full">
			...
			<div className="flex w-full">
				<Button
					onClick={() => {
						fetchReportKPIS(token.token);
					}}
					className="flex flex-wrap px-2 bg-green-400 w-full h-full">
					Generar Resumen de los proyectos <Excel />
				</Button>
			</div>
		</div>
	);
